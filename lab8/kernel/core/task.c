...
void task_init()
{
	...
	memset(task_pool, 0, sizeof(struct task) * TASK_POOL_SIZE);
	...
	uint64_t virtual_addr_start)
{
	...
	uint64_t mem_phys_addr = 0;
	uint64_t size = 0;
	
	pgdir = (pgd_t *)kmalloc_alloc(PAGE_SIZE);
	if (pgdir == NULL) {
	...
	for (uint64_t i = binary_start; i < binary_end; i += PAGE_SIZE, binary_virtual_addr += PAGE_SIZE) {
		...
		mem_phys_addr = KERNEL_VIRT_TO_PHYS(mem);		

		memcpy(mem, (void *) i, size);
	...
	uint64_t lr_address)
{
	...
	if (src->user_pgd_page != NULL) {
		...
		memset(pgd_dest, 0, PAGE_SIZE);

		pgd_src = src->user_pgd_page;
		
		if (mmu_copy_pgd(pgd_dest, pgd_src)) {
			goto Failed;
		}

		dest->user_pgd_page = pgd_dest;
