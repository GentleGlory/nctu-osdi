...
{
	uint64_t phys = KERNEL_VIRT_TO_PHYS((uint64_t) pte);
	uint64_t page_num = PHYS_TO_PAGE_FRAME_NUM(phys);
	uint64_t pte_page_nume;
	for (int i = 0; i < PTRS_PER_PTE; i++) {
		if ((*pte) != 0) {
			phys = (*pte) & PTE_ADDR_MASK;
			pte_page_nume = PHYS_TO_PAGE_FRAME_NUM(phys);
			page_free_by_page_num(pte_page_nume);
		}
...
{
	uint64_t phys = KERNEL_VIRT_TO_PHYS((uint64_t) pmd);
	uint64_t page_num = PHYS_TO_PAGE_FRAME_NUM(phys);
	pte_t *pte = NULL;
...
{	
	uint64_t phys = KERNEL_VIRT_TO_PHYS((uint64_t) pud);
	uint64_t page_num = PHYS_TO_PAGE_FRAME_NUM(phys);
	pmd_t *pmd = NULL;
...
{
	uint64_t phys = PAGE_FRAME_NUM_TO_PHYS(pgd_page->page_num);
	pgd_t * pgd = (pgd_t *) PHYS_TO_KERNEL_VIRT(phys);
	pud_t *pud = NULL;
